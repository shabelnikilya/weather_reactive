...
public class WeatherControl {
    ...
    public Mono<Weather> getMaxTCity() {
        return weathers.maxTempCity();
    }

    @GetMapping(value = "/upTemp/{temp}", produces = MediaType.TEXT_EVENT_STREAM_VALUE)
    public Flux<Weather> upTempCity(@PathVariable Integer temp) {
        Flux<Weather> data = weathers.citiesUpTemp(temp);
        Flux<Long> delay = Flux.interval(Duration.ofSeconds(3));
        return Flux.zip(data, delay).map(Tuple2::getT1);
    }
}
